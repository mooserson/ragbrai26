...
// Until it loads — or if it's missing — town-to-town straight lines stand in.
let trackPts = latlngs;
let trackCum = cumulativeMiles.slice();
fetch("route.geojson")
  .then(r => { if (!r.ok) throw new Error(r.status); return r.json(); })
  .then(geo => {
    const days = geo.features.slice().sort((a, b) => a.properties.day - b.properties.day);
    const pts = [];
    for (const f of days) for (const c of f.geometry.coordinates) pts.push([c[1], c[0]]);
